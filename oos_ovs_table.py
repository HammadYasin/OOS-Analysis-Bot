...
batch_details = pd.read_excel('batch_details_processed_ovs.xlsx')
contracts_dim = pd.read_ex



#creating Out of Stock Table
...
oos_table= oos_table.drop(columns = ['total_stock','average_consumption','covering_months','stock_category','overstock'])
oos_table = oos_table.dropna()
#oos_table.to_excel('oos_table.xlsx',index=False)



#creating OverStock Table
...
overstock_table = pd.merge(overstock_table,batch_details[['region_id','item_id','batch_id','expiry_date','unit_price']],on=['region_id',"item_id"],how='left')
...
overstock_table = overstock_table.dropna()
...
stock_transfer['expiry_cost'] = np.where(stock_transfer['expiry_date'] > date_after, 0,
                                         np.where(stock_transfer['required_quantity'] > stock_transfer['overstock'],
                                                  stock_transfer['overstock'] * stock_transfer['unit_price'],
